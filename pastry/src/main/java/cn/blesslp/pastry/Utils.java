...
public final class Utils {
    ...
    public static String comboURL(String baseURL,String url) {
        Utils.checkNotNull(baseURL, "NetAdapter必须设置BaseURL");
        Utils.checkNotNull(url, "API的接口必须指定url");
    ...
    static <T> void validateServiceInterface(Class<T> service) {
        if (!service.isInterface()) {
            throw new IllegalArgumentException("create方法传入参数必须是一个接口");
        }
        // Prevent API interfaces from extending other interfaces. This not only avoids a bug in
        // Android (http://b.android.com/58753) but it forces composition of API declarations which is
        // the recommended pattern.
        if (service.getInterfaces().length > 0) {
            throw new IllegalArgumentException("create传入参数不能继承自其他接口");
        }
    }
